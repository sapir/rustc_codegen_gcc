...
use gccjit::{FunctionType, RValue, ToRValue, Type, UnaryOp};
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    pub fn gcc_int_cast(&mut self, value: RValue<'gcc>, dest_typ: Type<'gcc>) -> RValue<'gcc> {
    ...
    pub fn gcc_urem(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
    ...
    pub fn gcc_not(&mut self, a: RValue<'gcc>) -> RValue<'gcc> {
        let typ = a.get_type();
        let operation =
            if typ.is_bool() {
    ...
    pub fn gcc_neg(&mut self, a: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        if self.supports_native_int_type(a_type) {
             ...
             */
            a
            // FIXME: __negti2 is not found by the linker.
            /*let a_int_type = self.get_int_type(a_type);
            let size =
                match a_int_type.bits {
                    64 => 'd',
    ...
    pub fn gcc_and(&mut self, a: RValue<'gcc>, mut b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
    ...
    pub fn gcc_or(&mut self, a: RValue<'gcc>, mut b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
        if self.supports_native_int_type_or_bool(a_type) && self.supports_native_int_type_or_bool(b_type) {
    ...
    pub fn gcc_add(&mut self, a: RValue<'gcc>, mut b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
        if self.supports_native_int_type_or_bool(a_type) && self.supports_native_int_type_or_bool(b_type) {
    ...
    pub fn gcc_mul(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
        if self.supports_native_int_type_or_bool(a_type) && self.supports_native_int_type_or_bool(b_type) {
            a * b
    ...
    pub fn gcc_sub(&mut self, a: RValue<'gcc>, mut b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
        if self.supports_native_int_type_or_bool(a_type) && self.supports_native_int_type_or_bool(b_type) {
            if a.get_type() != b.get_type() {
    ...
    pub fn gcc_udiv(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        let a_type = a.get_type();
        let b_type = b.get_type();
        if self.supports_native_int_type_or_bool(a_type) && self.supports_native_int_type_or_bool(b_type) {
    ...
    pub fn gcc_checked_binop(&mut self, oop: OverflowOp, typ: Ty<'_>, lhs: <Self as BackendTypes>::Value, rhs: <Self as BackendTypes>::Value) -> (<Self as BackendTypes>::Value, <Self as BackendTypes>::Value) {
...
impl<'gcc, 'tcx> CodegenCx<'gcc, 'tcx> {
    pub fn gcc_int(&self, typ: Type<'gcc>, mut int: i64) -> RValue<'gcc> {
        if self.supports_native_int_type_or_bool(typ) {
            ...
            let dest_size = int_type.bits as u32;
            let dest_element_size = int_type.element_size as u32;
            let mask = 1_i64.overflowing_shl(dest_element_size).0 - 1;
    ...
    pub fn gcc_uint(&self, typ: Type<'gcc>, mut int: u64) -> RValue<'gcc> {
        if self.supports_native_int_type(typ) {
            ...
            let dest_size = int_type.bits as u32;
            let dest_element_size = int_type.element_size as u32;
            let mask = 1_u64.overflowing_shl(dest_element_size).0 - 1;
            let native_int_type = self.get_unsigned_int_type_by_size(dest_element_size as u8).typ;
            let mut bit_set = 0;
            let mut values = vec![];
            // FIXME: if dest_element_size < dest_size, this loop is infinite.
    ...
    pub fn gcc_uint_big(&self, typ: Type<'gcc>, num: u128) -> RValue<'gcc> {
        ...
        if let Ok(num) = num64 {
            ...
            return self.gcc_int(typ, num as i64);
        }
